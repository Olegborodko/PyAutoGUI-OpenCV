...
def release_all_keys():
    """
    КРИТИЧНО ВАЖНА ФУНКЦИЯ для RDP.
    Освобождает все модификаторы, которые могли остаться зажатыми.
    Это предотвращает появление Input Capture Window overlay.
    """
    ...
    for k in keys:
        try:
            ...
            print(f"⚠️ Не удалось освободить клавишу {k}: {e}")
            pass
    
    # Дополнительная проверка: отправляем keyUp для всех модификаторов еще раз
    for k in keys:
        try:
            pyautogui.keyUp(k)
        except:
