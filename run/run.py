print("=" * 60)
print("ІНСТРУКЦІЇ ДЛЯ ПІДГОТОВКИ RDP ВІКНА")
print("=" * 60)

print("\nІНСТРУКЦІЯ:")
print("1. Відкрий віддалений рабочий стіл з потрібними вкладками")
print("2. Активуй вікно (клікни в нього), перше вікно з Latest Accession # кодом")
print("3. У тебе є 15 секунд для підготовки")

print("\n" + "=" * 60)

import time

# Відлік 15 секунд
for i in range(15, 0, -1):
    print(f"Старт через {i} секунд...")
    time.sleep(1)

print("\n✓ ГОТОВО! Починаю роботу...")

# Імпортуємо наші функції
from image_utils import SearchSettings, find_image, click_at_position
from text_utils import copy_text_from_position

def main_workflow():
    """Основна логіка роботи"""
    print("\n" + "=" * 60)
    print("ПОЧИНАЮ ОСНОВНУ РОБОТУ")
    print("=" * 60)
    
    # Налаштування пошуку зображення
    settings = SearchSettings(
        confidence=0.7,          # Чутливість розпізнавання
        grayscale=False,         # Ч/Б пошук
        blur=0,                  # Розмиття
        scales=[0.9, 1.0, 1.1],  # Масштаби для пошуку
        click_on="bottom",       # Клікати внизу зображення
        click_offset=(0, -3),    # На 3 пікселі нижче зображення
        max_attempts=3,          # Кількість спроб
        search_timeout=10.0      # Таймаут пошуку
    )
    
    # КРОК 1: Пошук та клік по зображенню
    print("\n🔍 КРОК 1: Шукаю зображення 'button.png'...")
    position = find_image("button.png", settings)
    
    if not position:
        print("❌ Не вдалося знайти зображення. Завершую роботу.")
        return False
    
    print(f"✅ Зображення знайдено за координатами: {position}")
    
    # КРОК 2: Клік по знайденій позиції
    print("\n🖱️ КРОК 2: Клікаю по знайденій позиції...")
    if not click_at_position(position):
        print("❌ Не вдалося виконати клік. Завершую роботу.")
        return False
    
    print("✅ Клік виконано успішно!")
    
    # КРОК 3: Копіювання тексту з позиції
    print("\n📋 КРОК 3: Копіюю текст з позиції...")
    copied_text = copy_text_from_position(position[0], position[1])
    
    if not copied_text:
        print("❌ Не вдалося скопіювати текст. Завершую роботу.")
        return False
    
    print(f"✅ Текст успішно скопійовано!")
    print(f"📄 Зміст тексту: {copied_text[:100]}..." if len(copied_text) > 100 else f"📄 Зміст тексту: {copied_text}")
    
    # Тут можна додати додаткову обробку скопійованого тексту
    # Наприклад, зберегти у файл, проаналізувати тощо
    
    return True

def handle_error(error_message: str):
    """Обробка помилок"""
    print("\n" + "=" * 60)
    print("🚨 ПОМИЛКА В РОБОТІ")
    print("=" * 60)
    print(f"Повідомлення про помилку: {error_message}")
    print("Робота завершена з помилкою.")
    print("=" * 60)

def main():
    """Головна функція"""
    try:
        success = main_workflow()
        
        if success:
            print("\n" + "=" * 60)
            print("✅ РОБОТУ УСПІШНО ЗАВЕРШЕНО!")
            print("=" * 60)
        else:
            print("\n" + "=" * 60)
            print("❌ РОБОТУ ЗАВЕРШЕНО З ПОМИЛКАМИ")
            print("=" * 60)
            
    except Exception as e:
        handle_error(str(e))
        return False
    
    return True

if __name__ == "__main__":
    main()